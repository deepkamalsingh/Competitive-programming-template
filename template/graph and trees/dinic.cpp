/*
	* Description :- dinic algo for max flow
	* Source :- https://codeforces.com/contest/1521/submission/115574965
	* Verf :- https://cses.fi/problemset/result/3438946/
*/
...
struct dinic{
	struct edge{
		...
		edge(int _node,int _rev,int _capacity,int _id = -1) : node(_node), rev(_rev), capacity(_capacity), id(_id) {}
	};
	...
	void bfs_check(int node,int potential_dist){
		if(potential_dist < dist[node])
			dist[q[q_end++] = node] = potential_dist;
	}
